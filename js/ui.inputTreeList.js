 ...
 * |         |            |
 * |         |            | * change name from categorySelector to inputTreeList
 * |         |            |
 * |         |            |
 * |         |            |
 * |         |            |
      ...
      setDisplayStatus : function (object, value)
        {
          ...
          if((!properties.initialized || options.displayStatus!=value) && (value==true || value==false))
          {
            ...
            {
              object.find('.ui-inputTreeList-status').show();
            }
            else
            {
              object.find('.ui-inputTreeList-status').hide();
            }
          }
          return(options.displayStatus);
        },
            ...
            else if(target[4]!=null)
            {
              switch(target[5])
              {
                case '+':
                  ...
                  break;
                case '>':
                  objects.list.find('[catId='+target[4]+'] + ul').find('.ui-inputTreeList-expandable-item, .ui-inputTreeList-collapsable-item').each(applyExpandCollapse);
                  objects.list.find('[catId='+target[4]+']').each(applyExpandCollapse);
                  break;
                case '<':
      ...
      setValue : function (object, value, trigger)
        {
              ...
              objects.list.find('.ui-inputTreeList-item').each(
                function ()
                {
                  ...
                  if(apply)
                  {
                    ...
                    {
                      $this.removeClass('ui-inputTreeList-selected-item');
                    }
                    else
                    {
                      $this.addClass('ui-inputTreeList-selected-item');
            ...
            case ':none':
              ...
              objects.list.find('.ui-inputTreeList-selected-item').each(
                function ()
                {
                  ...
                  if(apply) $this.removeClass('ui-inputTreeList-selected-item');
                }
              );
              privateMethods.setValue(object, [], false);
              return(false);
          ...
          if(!options.multiple && (!properties.initialized || properties.index!=index) && index>-1)
          {
            ...
            objects.list.find('.ui-inputTreeList-selected-item').removeClass('ui-inputTreeList-selected-item');
            objects.list.find('[catId="'+value+'"]').addClass('ui-inputTreeList-selected-item');
            title=privateMethods.getParentName(object, objects.list.find('[catId="'+value+'"] div.ui-inputTreeList-name')).replace('&amp;', '&').replace('&gt;', '>').replace('&lt;', '<');
            ...
            if(trigger && options.change) object.trigger('inputTreeListChange', [properties.categories[properties.index].id]);
          ...
          else if(options.multiple)
          {
            ...
            for(var i=0;i<index.length;i++)
            {
              ...
              {
                item.removeClass('ui-inputTreeList-selected-item');
              ...
              {
                item.addClass('ui-inputTreeList-selected-item');
          ...
          $.ajax(
            {
              ...
              data:
                {
                  ajaxfct:options.userMode+'.inputTreeList.getList',
      ...
      listAddItems : function (object, listItems, parent)
        {
          ...
          for(var i=0;i<listItems.length;i++)
          {
            ...
            if(options.displayStatus)
            {
              status="<div class='ui-inputTreeList-status'>"+properties.labelStatus[listItems[i].status]+"</div>";
            }
            else
            {
              status="";
            ...
            var spaceWidth = (options.iconWidthEC+listItems[i].level*options.levelIndent),
                li=$('<li/>',
                      {
                        'class':'ui-inputTreeList-item',
                        html:"<div>"+status+"<div class='ui-inputTreeList-name'>"+listItems[i].name+"</div></div>",
